/* alarme com notificação no telegram
 *
 * Author: Gabriel Pinós Camargo
 * e-mail: gabriel-camargo01@uergs.edu.br
 * Date: 25/09/2022
 * MIT License
...
/*WIFI configuration*/
#define ESP_WIFI_SSID      "SSIDDOSEUWIFI"
#define ESP_WIFI_PASS      "SENHADOSEUWIFI"
...
void Pin_As_IRAM_Input(uint8_t pin, uint8_t *interrupt_flag) {
    ...
    gpio_isr_handler_add(pin, Pin_ISR_Handler, (void*) interrupt_flag); 
}
...
static void https_telegram_sendMessage_native_get(void) {
	...
	*/
    ...
    ESP_LOGW(TAG3, "Iniciando");
    esp_http_client_handle_t client = esp_http_client_init(&config);
    ESP_LOGW(TAG3, "Enviar mensagem para um chat");
    ESP_LOGW(TAG3, "Open");
    ...
    strcat(url,"A%20Porta%20foi%20aberta");
    ESP_LOGW(TAG3, "url string: %s",url);
...
void app_main(void)
{
    bool readSwitch = false, lastReadSwitch = false;
    //Initialize NVS
    ...
    Pin_As_IRAM_Input(INTERRUPTION_PIN, &readSwitch);
    ...
    while (1)
    {
        if(readSwitch && lastReadSwitch == false) {
            xTaskCreatePinnedToCore(&enviar_mensagem_telegram, "enviar_mensagem_telegram", 8192*12, NULL, 5, NULL,1);
            lastReadSwitch = true;
            readSwitch = false;
        } else {
            lastReadSwitch = false;
        }
